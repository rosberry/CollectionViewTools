...
public final class TypeCollectionViewCellItem<U: Equatable, T: UICollectionViewCell>: CollectionViewDiffCellItem {

    public lazy var reuseType = ReuseType.classWithIdentifier(T.self, identifier: diffIdentifier)
    public lazy var diffIdentifier: String = "\(String(describing: type(of: self)))"
    public let object: U
    ...
    public func isEqual(to item: DiffItem) -> Bool {
        guard let cellItem = item as? TypeCollectionViewCellItem<U, T> else {
            return false
        }
        return cellItem.object == object
    }
}
